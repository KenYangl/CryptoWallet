...
final public class AES256CBC {
    public class func encryptString(_ str: String, key: String) -> String? {
        if !str.isEmpty && key.length == 64 {
            let ivData = randomText(16).data(using: .utf8)!
            let keyData = Data(hexString: key)!

            guard let encryptedString = try? aesEncrypt(str, key: keyData, iv: ivData) else {
                print("an error occured while encrypting")
                return nil
            }
            return "\(encryptedString)--\(ivData.base64EncodedString())"
        }
        return nil
    }

    public class func decryptString(_ str: String, key: String) -> String? {
        if str.length > 16 && key.length == 64 {
            let keyData = Data(hexString: key)!

            let components = str.components(separatedBy: "--")
            let encryptedString = components[0]
            let ivData = Data(base64Encoded: components[1])!

            guard let decryptedString = try? aesDecrypt(encryptedString, key: keyData, iv: ivData) else {
    ...
    fileprivate class func aesEncrypt(_ str: String, key: Data, iv: Data) throws -> String {
    ...
    fileprivate class func aesDecrypt(_ str: String, key: Data, iv: Data) throws -> String {
        // let keyData = key.data(using: String.Encoding.utf8)!
        // let ivData = iv.data(using: String.Encoding.utf8)!
        let keyData = key
        let ivData = iv
...
private func arrayOfBytes<T>(value: T, length: Int? = nil) -> Array<UInt8> {
    ...
    for j in 0..<min(MemoryLayout<T>.size, totalBytes) {
        ...
        valuePointer.deinitialize(count: j)
    }
